...
public:
  ...
  //removes one child from children, but doesn't delete it.
  ...
  //prints kids's parent name (only for debuging purposes)
  void printKidssParent()
  {
	  std::cout << "\nMy name: " << name() << "\n";
	  for (auto o : _children)
		  std::cout << "Kid name: " << o->name() << ", kid's parent name: " << o->parent()->name() << "; ";
	  std::cout << "\n";
  }
